...
class SFT(nn.Module):
    def __init__(self, x_nc, prior_nc=1, ks=3, nhidden=128):
        ...
        self.mlp_gamma = zero_module(nn.Conv2d(nhidden, x_nc, kernel_size=ks, padding=pw))
        self.mlp_beta = zero_module(nn.Conv2d(nhidden, x_nc, kernel_size=ks, padding=pw))
...
class SFTResblk(nn.Module):
    def __init__(self, x_nc, prior_nc, ks=3):
        super().__init__()
        self.conv_0 = zero_module(nn.Conv2d(x_nc, x_nc, kernel_size=3, padding=1))
        self.conv_1 = zero_module(nn.Conv2d(x_nc, x_nc, kernel_size=3, padding=1))
