...
from data_compression.distributions.common import Normal, DeepNormal, DeepMixtureNormal
...
class UniformNoiseAdapter(nn.Module):
    ...
    def log_prob(self, y):
        ...
        try:
            ...
            log_prob = self._log_prob_with_cdf(y)
        return ops.lower_bound(log_prob, math.log(1e-9))  ## 2
...
class NoisyDeepMixtureNormal(UniformNoiseAdapter):
    """Gaussian distribution with additive i.i.d. uniform noise."""

    def __init__(self, **kwargs):
        super().__init__(DeepMixtureNormal(**kwargs))
