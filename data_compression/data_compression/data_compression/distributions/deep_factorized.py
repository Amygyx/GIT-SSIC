...
class Factorized(Distribution):
    ...
    def _reshape_value(self, value):
        ...
        if len(shape) == 4:
            ## only for (B, C, H, W)
            assert value.shape[-2-len(self.batch_shape):-2] \
                   == self.batch_shape
            return value.view(*shape[:-2], 1, -1)
        else:
            ## for (*, *batch_shape)
            assert value.shape[-len(self.batch_shape):] \
                   == self.batch_shape
...
class DeepFactorized(DeepDistribution):
    ...
    def _make_parameters(self, batch_shape):
        ...
        for i in range(len(self.num_filters) + 1):
            ...
            matrix = nn.Parameter(torch.Tensor(1, *batch_shape, _filters[i + 1], _filters[i]))
            nn.init.constant_(matrix, init)
            self.matrices.append(matrix)

            bias = nn.Parameter(torch.Tensor(1, *batch_shape, _filters[i + 1], 1))
            nn.init.uniform_(bias, -0.5, 0.5)
            self.biases.append(bias)

            if i < len(self.num_filters):
                factor = nn.Parameter(torch.Tensor(1, *batch_shape, _filters[i + 1], 1))
